...
func (s *basePropConstSolver) tryToUpdateEQList(col *Column, con *Constant) (bool, bool) {
	value, err := con.Eval(s.ctx.GetEvalCtx(), chunk.Row{})
	if err != nil {
		return false, false
	}

	if value.IsNull() && ConstExprConsiderPlanCache(con, s.ctx.IsUseCache()) {
		return false, true
	}
	id := s.getColID(col)
	oldCon := s.eqList[id]
	if oldCon != nil {
		...
		oldValue, err := oldCon.Eval(evalCtx, chunk.Row{})
		if err != nil {
			return false, false
		}
		res, err := oldValue.Compare(evalCtx.TypeCtx(), &value, collate.GetCollator(col.GetType(s.ctx.GetEvalCtx()).GetCollate()))
