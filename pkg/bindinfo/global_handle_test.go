...
func TestBindingLastUpdateTime(t *testing.T) {
	...
	noDBDigest := bindinfo.NormalizeDigestForBinding(stmt, "", true)
...
func TestBindParse(t *testing.T) {
	...
	_, digest := parser.NormalizeDigestForBinding(originSQL, false)
...
func TestSetBindingStatusWithoutBindingInCache(t *testing.T) {
	...
	_, sqlDigest := parser.NormalizeDigestForBinding("select * from `test` . `t` where `a` > ?", false)
...
func TestNormalizeStmtForBinding(t *testing.T) {
	...
	for _, test := range tests {
		...
		n := bindinfo.NormalizeStmtForBinding(stmt, "", false)
		digest := bindinfo.NormalizeDigestForBinding(stmt, "", true)
...
func utilNormalizeWithDefaultDB(t *testing.T, sql string) (stmt ast.StmtNode, normalized, digest string) {
	...
	require.NoError(t, err)
	normalized = bindinfo.NormalizeStmtForBinding(stmt, "test", false)
	digest = bindinfo.NormalizeDigestForBinding(stmt, "test", false)
	return stmt, normalized, digest
}
