...
func (s *testChunkSuite) TestVecChunk(c *check.C) {
	...
	chk := NewChunkWithCapacity(fieldTypes, 1024)

	for i := 0; i < 1024; i++ {
		chk.AppendInt64(0, int64(i))
		chk.AppendFloat32(1, float32(i))
		chk.AppendString(2, fmt.Sprintf("%v", i))
	}

	i64s := chk.Vector(0).Int64()
	c.Assert(len(i64s), check.Equals, 1024)
	for i := 0; i < 1024; i++ {
		c.Assert(i64s[i], check.Equals, int64(i))
	}

	f32s := chk.Vector(1).Float32()
	c.Assert(len(f32s), check.Equals, 1024)
	for i := 0; i < 1024; i++ {
		c.Assert(f32s[i], check.Equals, float32(i))
	}

	strs := chk.Vector(2).String()
	c.Assert(len(strs), check.Equals, 1024)
	for i := 0; i < 1024; i++ {
		c.Assert(strs[i], check.Equals, fmt.Sprintf("%v", i))
	}
