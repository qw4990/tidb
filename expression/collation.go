...
func DeriveCollationFromExprs(exprs ...Expression) (charset, collation string, length int, err error) {
	if len(exprs) == 0 {
		return "", "", 0, errors.New("no expression to derive collation")
	}
	...
	tp := e.GetType()
	return tp.Charset, tp.Collate, tp.Flen, nil
}
