...
func (p *PhysicalIndexReader) getPlanCostVer2(taskType property.TaskType, option *PlanCostOption) (costVer2, error) {
	...
	seekCost := seekCostVer2(estimateNumTasks(p.indexPlan), seekFactor)
...
func (p *PhysicalTableReader) getPlanCostVer2(taskType property.TaskType, option *PlanCostOption) (costVer2, error) {
	...
	netCost := netCostVer2(rows, rowSize, netFactor)
	seekCost := seekCostVer2(estimateNetSeekCost(p.tablePlan), seekFactor)
...
// double-read-cost = double-read-tasks * seek-factor
...
func (p *PhysicalIndexLookUpReader) getPlanCostVer2(taskType property.TaskType, option *PlanCostOption) (costVer2, error) {
	...
	indexSeekCost := seekCostVer2(estimateNumTasks(p.indexPlan), seekFactor)
	...
	tableSeekCost := seekCostVer2(estimateNumTasks(p.tablePlan), seekFactor)
	...
	doubleReadSeekCost := seekCostVer2(doubleReadTasks, seekFactor)

	p.planCostVer2 = sumCostVer2(indexSideCost, divCostVer2(sumCostVer2(tableSideCost, doubleReadSeekCost), doubleReadConcurrency))
	p.planCostInit = true
	return p.planCostVer2, nil
}
...
func (p *PhysicalIndexMergeReader) getPlanCostVer2(taskType property.TaskType, option *PlanCostOption) (costVer2, error) {
	...
	if tablePath := p.tablePlan; tablePath != nil {
		...
		tableSeekCost := seekCostVer2(estimateNumTasks(tablePath), seekFactor)
	...
	for _, indexPath := range p.partialPlans {
		...
		indexSeekCost := seekCostVer2(estimateNumTasks(indexPath), seekFactor)
		indexChildCost, err := indexPath.getPlanCostVer2(taskType, option)
		if err != nil {
			return zeroCostVer2, err
		}

		indexSideCosts = append(indexSideCosts, divCostVer2(sumCostVer2(indexNetCost, indexSeekCost, indexChildCost), distConcurrency))
	}

	p.planCostVer2 = sumCostVer2(tableSideCost, sumCostVer2(indexSideCosts...))
	p.planCostInit = true
	return p.planCostVer2, nil
}
...
func (p *PhysicalMergeJoin) getPlanCostVer2(taskType property.TaskType, option *PlanCostOption) (costVer2, error) {
	...
	filterCost := sumCostVer2(filterCostVer2(leftRows, p.LeftConditions, cpuFactor),
		filterCostVer2(rightRows, p.RightConditions, cpuFactor))
	groupCost := sumCostVer2(groupCostVer2(leftRows, cols2Exprs(p.LeftJoinKeys), cpuFactor),
...
func (p *PointGetPlan) getPlanCostVer2(taskType property.TaskType, option *PlanCostOption) (costVer2, error) {
	...
	seekCost := divCostVer2(seekCostVer2(1, seekFactor), 20) // 20 times faster than general request
...
func (p *BatchPointGetPlan) getPlanCostVer2(taskType property.TaskType, option *PlanCostOption) (costVer2, error) {
	...
	netCost := netCostVer2(rows, rowSize, netFactor)
	seekCost := divCostVer2(seekCostVer2(1, seekFactor), 20) // in one batch
